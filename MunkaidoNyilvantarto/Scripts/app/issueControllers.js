(function () {
    ...
    .controller('issueDetailsCtrl', ['$scope', '$routeParams', '$http', '$window', 'AuthService', 'alertService',
        function ($scope, $routeParams, $http, $window, AuthService, alertService) {
            ...
            function refreshSpentTimes() {
                $http.get('/SpentTime/GetSpentTimesByIssue/' + $routeParams.issueId).then(function (resp) {
                    if (resp.data.Succeeded) {
                        $scope.issue.SpentTimes = resp.data.Data;
            ...
            $scope.deleteSpentTime = function (id) {
                $http.post('/SpentTime/DeleteSpentTime/' + id, null).then(function (resp) {
                    if (resp.data.Succeeded) {
                        refreshSpentTimes();
                    }
                    else {
